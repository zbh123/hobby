...
def RNN(X, weights, biases):
    ...
    # X（128,28,28）128个图片，图片大小28*28需要先将X转换成如下格式
    # X ==> (128 batch * 28 steps, 28 inputs)
    X = tf.reshape(X, [-1, n_inputs])

    # into hidden
    # X_in = (128 batch * 28 steps, 128 hidden)
    # 下式为矩阵乘法得出的是二维矩阵，需要将其转为三维的形式，才能进入到RNN的cell层
    ...
    # basic LSTM Cell.定义RNN的cell
    if int((tf.__version__).split('.')[1]) < 12 and int((tf.__version__).split('.')[0]) < 1:
        # n_hidden_units隐藏层， forget_bias初始的biases为1.0，state_is_tuple每一步的结果都是一个state，保存成元组的形式
    ...
    # outputs是list，保存每一步的结果， final_state是最后一次的结果， initial_state初始化
