...
def generate_sql(table_name, value):
    ...
    if table_name == 'wind_dcm':
        sql = "INSERT INTO " + table_name + " (`organization`, total_amount, total_ranking, market_share, `number`," \
                                        ...
                                        " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + now_time + "' and `organization` = '" + value[0] + "')"
        ...
        sql = "INSERT INTO " + table_name + " (`code`, bond_abbr, announce_date_late, enterprise, audit_status, " \
                                            "pre_disclosed, listing_board, ipo_theme, ipo_theme_detail," \
                                            " industry_csrc, pre_issued, pre_total, place_registration," \
                                            " sponsor, sponsor_person, account_firm, accountant, law_firm," \
                                            "lawyer, change_status, announcement_date_f, website, email, phone, " \
                                            "address, `current_time`) " \
                                            "SELECT %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, " \
                                            "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s"\
                                            " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + now_time + "' and `code` = '" + value[0] + "')"
        ...
        sql = "INSERT INTO " + table_name + " (index_l, application_type, enterprise, stock_code, sponsor, " \
                                            "accept_date, audit_status, remark, `current_time`) " \
         ...
         sql = "INSERT INTO " + table_name + " (index_l, enterprise, sponsor, account_firm, law_firm, accept_date, " \
                                            "audit_status, spot_check, remark, `current_time`) " \
        ...
        sql = "INSERT INTO " + table_name + " (index_l, fullname_issuer, audit_status, place_registrate, srci, sponsor," \
                                            "  law_firm, account_firm, update_date, accept_date, `current_time`) " \
...
if __name__ == '__main__':
    path_db = {'债权承销排名(不含可转债)': ['wind_dcm', 3], '债权承销排名(不含三债)': ['wind_dcm_without_gb', 3],
               '科创板IPO审核申报企业情况': ['wind_star', 1], '股权投行承销排名_上市日': ['wind_ecm_list', 2],
    ...
    # path = r'D:\0RPA\计划财务部\投行业务\20200720\科创板IPO审核申报企业情况.xls'
    ...
    # store_to('investment_banks', 'shenzhen_ipo', path)
