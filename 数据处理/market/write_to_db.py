...
def generate_sql(table_name, value):
    ...
    sql = "INSERT INTO " + table_name + " (num_list_innovate, num_list_base, rank_market, num_market, `current_time`) " \
                                        "SELECT %s, %s, %s, %s, %s"\
                                        " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + now_time + "' and num_list_innovate = '" + \
              value[0] + "')"
...
if __name__ == '__main__':
    path_db = {'做市商': 'market_maker'}

    path = r'D:\0RPA\计划财务部\做市业务'
    now_time = time.strftime("%Y%m%d", time.localtime(time.time()))
    ...
    for fileName in fileList:
        ...
        if not os.path.isdir(fileAbsPath):
            ...
            for key, value in path_db.items():
                if base_dir in key:
                    store_to('market', value, fileAbsPath)
