...
def generate_sql(table_name, value):
    ...
    sql = "INSERT INTO " + table_name + " (index_1, company, province, industry, name_bonders, " \
                                        "project_leader_bonder, law_firm, lawyer, account_firm, cpa, list_issue_ats," \
                                        "audit_status, accept_date, one_feedback, one_reply, two_feedback, two_reply, " \
                                        "three_feedback, three_reply, four_feedback, four_reply, five_feedback, " \
                                        "five_reply, six_feedback, six_reply, seven_feedback, seven_reply," \
                                        " eight_feedback, eight_reply, `current_time`) " \
...
if __name__ == '__main__':
    path_db = {'全国股份转让系统在审申请挂牌企业基本情况表': 'nsts_apply_list'}

    path = r'D:\0RPA\计划财务部\中小企业'
    now_time = time.strftime("%Y%m%d", time.localtime(time.time()))
    ...
    for fileName in fileList:
        ...
        if not os.path.isdir(fileAbsPath):
            ...
            for key, value in path_db.items():
                if base_dir in key:
                    store_to('SME', value, fileAbsPath)
