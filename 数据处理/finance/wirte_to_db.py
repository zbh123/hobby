...
def generate_sql(table_name, value):
    ...
    '''
     `current_time`
    '''
    sql = "INSERT INTO " + table_name + " (indicator_name, total_industry, company_value, company_rank, " \
                                        "number_statistic, financial_time, `current_time`) " \
                                        "SELECT %s, %s, %s, %s, %s, %s, %s"\
                                        " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + now_time + "' and indicator_name = '" + \
              value[0] + "')"
...
if __name__ == '__main__':
    path_db = {'CISP数据': 'industry_data'}

    path = r'D:\0RPA\计划财务部\计划财务'
    now_time = time.strftime("%Y%m%d", time.localtime(time.time()))
    ...
    for fileName in fileList:
        ...
        if not os.path.isdir(fileAbsPath):
            ...
            for key, value in path_db.items():
                if base_dir in key:
                    store_to('PFGH', value, fileAbsPath)
